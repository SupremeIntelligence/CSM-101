...
def get_channel_members(bot: commands.Bot, guild_id: int, channel_id: int):
...
def sync_users(bot: commands.Bot, guild_id: int, channel_id: int, users: dict[int, user.User]):
    guild = bot.get_guild(guild_id)
    channel = bot.get_channel(channel_id)
    if guild is None:
        logger.critical("Сервер не найден")
